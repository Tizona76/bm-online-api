...
def _payment_background_data_uri() -> str:
    path = os.path.join(os.path.dirname(__file__), "payment_assets", "fond_match.png")
    try:
...
def _payment_static_page(title: str, body: str) -> Response:
    html = f"""<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  ...
  <title>{title}</title>
  <style>
    body {{ margin: 0; min-height: 100vh; display: flex; align-items: center; justify-content: center; font-family: Arial, sans-serif; background: #06152f; color: #fff; text-align: center; }}
    main {{ max-width: 560px; padding: 32px; }}
    h1 {{ margin: 0 0 16px; font-size: 32px; }}
    p {{ font-size: 18px; line-height: 1.45; color: #dbeafe; }}
    button {{ margin-top: 18px; border: 0; border-radius: 8px; padding: 12px 18px; background: #20b455; color: #fff; font-size: 16px; cursor: pointer; }}
  </style>
</head>
<body>
  <main>
...
def payment_success():
    bg_url = _payment_background_data_uri()
    background_image = f"url('{bg_url}') center / cover no-repeat," if bg_url else ""
    html = f"""<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    body {{
      ...
      text-align: center;
      background:
        linear-gradient(rgba(3, 8, 20, 0.58), rgba(3, 8, 20, 0.72)),
        {background_image}
        #06152f;
    }}
    main {{ width: min(680px, calc(100vw - 40px)); padding: 56px 20px; }}
    .ball {{ font-size: 54px; line-height: 1; margin-bottom: 26px; }}
    h1 {{ margin: 0 0 18px; font-size: clamp(34px, 6vw, 56px); font-weight: 800; letter-spacing: 0; }}
    p {{ margin: 10px auto 0; max-width: 560px; font-size: clamp(18px, 2.4vw, 22px); line-height: 1.5; color: rgba(238, 246, 255, 0.94); }}
    button {{
      margin-top: 38px;
      border: 0;
      border-radius: 10px;
    ...
    #closeHint {{ display: none; margin-top: 18px; font-size: 17px; color: rgba(238, 246, 255, 0.86); }}
  </style>
</head>
<body>
  <main>
    ...
    <p>Your Club Tokens are being synchronized.</p>
    <button type="button" onclick="window.close(); setTimeout(function(){{ document.getElementById('closeHint').style.display='block'; }}, 250);">Return to Basket Manager</button>
    <div id="closeHint">Return to your Basket Manager tab.</div>
  </main>
</body>
</html>"""
    return Response(content=html, media_type="text/html; charset=utf-8")
...
def payment_cancel():
    return _payment_static_page(
        "Payment cancelled",
        "No payment was made.<br>You can close this tab and return to Basket Manager.",
    )
...
PAYMENT_SUCCESS_URL_DEFAULT = "https://api.basketmanager-game.com/payment/success?session_id={CHECKOUT_SESSION_ID}"
PAYMENT_CANCEL_URL_DEFAULT = "https://api.basketmanager-game.com/payment/cancel"

def _payment_page_url_from_env(name: str, default_url: str, required_path: str) -> str:
    raw = (os.environ.get(name, "") or "").strip()
    if raw and required_path in raw and "itch.io" not in raw.lower():
        return raw
    return default_url
...
def _funnel_log(event_name: str, profile_uuid: Optional[str] = "", team_name: Optional[str] = "", ip: Optional[str] = None, meta: Optional[Dict[str, Any]] = None) -> None:
    try:
        eng = _lb_get_engine()
        if eng is None:
            return
...
def funnel_team_name_created(p: FunnelTeamNamePayload, request: Request):
    ...
    _funnel_log("team-name-created", user_id, p.team_name or "", ip, p.meta)
    return {"ok": True}
